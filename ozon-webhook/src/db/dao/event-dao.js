 ...
 */
export function insertEvent({ message_type, idempotency_key, seller_id, posting_number, product_id, sku, raw_payload }) {
  const db = getDb();
  const received_at = new Date().toISOString();
  try {
    const stmt = db.prepare(`
      INSERT INTO ozon_push_events
        (message_type, idempotency_key, seller_id, posting_number, product_id, sku, raw_payload, status, received_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)
    `);
    stmt.run(message_type, idempotency_key, seller_id, posting_number, product_id, sku, raw_payload, received_at);
