 ...
 *
 * 测试覆盖:
 *   1. SW 侧 modal 响应解析逻辑(从 doFetch 闭包抽出的纯函数)
 *   2. stale 判定逻辑(4h TTL)
...
// ============================================================
// 1. SW 侧 modal 响应解析逻辑(从 service-worker.js doFetch 抽出)
// ============================================================
// 来源:service-worker.js L2286-2378
// 原样复制,便于单元测试 doFetch 闭包内的逻辑

/**
 * 模拟 SW 侧 doFetch 内的 followSell modal 解析逻辑
...
test('★content 侧空 widgetStates → throw(走 failure tracking)', () => {
  assert.throws(
    () => contentParseFollowSellModal({ widgetStates: {} }),
    /empty widgetStates/,
    'content 侧应该 throw'
  );
...
test('★SW 侧空 widgetStates → 返回 no-sellers(不 throw,写缓存)', () => {
  const swResult = swParseFollowSellModal(
    { ok: true, status: 200, jsonData: { widgetStates: {} } },
    '123'
  );
...
test('主产品页 URL 正常匹配 -\\d{5,}/ 正则', () => {
  const productUrl = new URL(
    'http://localhost:7777/api/composer-api.bx/page/json/v2?url=' +
      encodeURIComponent('/product/test-slug-100001/')
  );
...
testAsync('★完整流程:反爬 stub → 缓存零跟卖 → 4h 内不再重试', async () => {
  ...
  const cache = followSellData
    ? { data: followSellData, fetchedAt: Date.now(), stale: false }
    : null;
