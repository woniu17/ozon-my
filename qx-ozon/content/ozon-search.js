(function () {
  ...
  //
  // 把本地桶 status='local' 的 SKU 批量全源采集到 collect_box_v2。
  // 复用 collectBySkus 拉 sv + video + richContent,组装 CollectedVariant[](字段级 source 标记),
  // 逐 SKU 推送 pushCollectBoxV2(后端按 (store_id, sku) upsert)。
  // DOM 专属字段(breadcrumbs/aspectValues/seller/详细 statistics)降级为 null。
      ...
      try {
        const result = await window.JZSkuCollect.collectBySkus(skus, {
          ...
          onProgress: (done, total) => {
            collectorPanel?.toast?.(`全源采集 ${done}/${total}`, 'info', 1500);
          },
        });
  ...
  // 暴露给 ozon-data-panel.js(PDP 页采集器面板)跨 content_scripts 调用
