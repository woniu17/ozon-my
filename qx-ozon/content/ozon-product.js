(function () {
  ...
  // ── 采集函数已迁移到 collect/content/collect-pdp.js ──
  // collectAllVariants / pushCollectBoxV2FromCollected / performProductCollect
  // 通过 window.__jzPdpCollect 暴露,本文件按钮 handler 经该对象调用。
  // B 类共用函数(extractProductData/extractAspectVariants 等)通过
  // window.__jzPdpCollectExtract 桥接对象暴露给 collect-pdp.js。
  ...
  // ── B 类共用函数桥接:暴露给 collect/content/collect-pdp.js ──
  // collect-pdp.js 的 collectAllVariants/performProductCollect 通过
  // window.__jzPdpCollectExtract.* 访问这些函数(惰性查找,调用时已就绪)。
  window.__jzPdpCollectExtract = {
    extractProductData,
    extractAspectVariants,
    extractRawAspects,
    jzExpandVariantsViaModal,
    extractBreadcrumbs,
    ...
    getMainImageUrl,
    logProductSummary,
    detectCurrencyFromPriceStr: _detectCurrencyFromPriceStr,
    rubToCny: _rubToCny,
    detectPageCurrency: _detectPageCurrency,
    fetchVariantGallery,
  ...
  function createActionBar() {
    ...
    const collectBtn = createActionButton(_ICONS.collect, '一键采集', async () => {
      ...
      try {
        const result = await window.__jzPdpCollect.collectAllVariants(collectBtn);
  ...
  function createSidebarDataCard() {
    ...
    card.querySelector('[data-action="edit-list"]')?.addEventListener('click', async () => {
      ...
      try {
        // 复用 collect-pdp.js 的 performProductCollect(含 ensurePdpState 预热、字段校验、
        // searchVariants、视频转存、富内容注入、v2 推送等全套采集逻辑)。
        // edit-list 按钮只负责采集后的跳转,不再维护第三套重复采集代码。
        editBtn.textContent = '⏳ 采集中...';
        const result = await window.__jzPdpCollect.performProductCollect();
    ...
    card.querySelector('[data-action="collect-one"]')?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        // 该按钮上;单/无变体页内部自动委托单采。
        const result = await window.__jzPdpCollect.collectAllVariants(btn);
  ...
  async function toggleFollowSellPanel(btn) {
    ...
    try {
      ...
      // 是否能跑 Phase B(JZSkuCollect 由 manifest 注入 collect/content/sku-collect.js,
  ...
  async function init() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      // triggerCollectFromPopup 已迁移到 collect/content/collect-pdp.js 独立 listener
      return true;
    });
