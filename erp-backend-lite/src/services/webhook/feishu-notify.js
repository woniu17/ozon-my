...
import { translateCancelReason } from '../cancel-reason-i18n.js';
...
/**
 * 取消原因俄语→中文翻译(实时取消推送用)
 * 映射表已抽取到 services/cancel-reason-i18n.js(与订单处理列表共用单一数据源)
 */
export function formatCancelReason(reason) {
  if (reason == null) return null;
  return translateCancelReason(reason) ?? String(reason);
}
