...
import { claimPendingEvents, markSuccess, markFailed, markDead } from '../../db/dao/sqlite/event-dao.js';
...
export function startEventPoller() {
  if (timer) return;
  logger.info({ intervalMs: config.webhook.poller.intervalMs, concurrency: config.webhook.poller.concurrency }, '启动 Event Poller');
  // 启动后立即跑一次,再进入定时
