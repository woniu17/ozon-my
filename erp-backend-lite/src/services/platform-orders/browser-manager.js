// 平台订单浏览器管理器(2026-09,平台订单获取 M1)
// cloakbrowser 单例:懒启动 / 空闲回收 / PID 锁 / 崩溃重建
//
// 职责:
//  - 懒启动:首个订单请求才 launchPersistentContext(.linqx-profile),冷启动约 15-25s
...
//
// 设计文档: docs/平台订单获取后端化-概要设计.md §3/§6
...
const lockFile = join(dirname(config.platformProfileDir), `${basename(config.platformProfileDir)}.platform-orders.lock`);

let ctx = null;                 // BrowserContext 单例
...
function acquireLock() {
  ...
  try {
    ...
    if (Number.isFinite(pid) && pid > 0 && pid !== process.pid) {
      if (isPidAlive(pid)) {
        throw new ApiError('PROFILE_LOCKED', `profile 正被本服务其它进程(PID ${pid})占用,锁文件: ${lockFile}`, { status: 409 });
      }
      stale = true;
    }
  } catch (e) {
  ...
  if (stale) logger.info('[platform-orders] 已覆盖残留锁文件(进程已死)');
}
...
async function doLaunch() {
  ...
  try {
    const c = await launchPersistentContext({
      userDataDir: config.platformProfileDir,
    ...
    ctx = c;
    logger.info(
      { profile: config.platformProfileDir, headless: config.platformBrowserHeadless, restoredTabs: restored.length },
    ...
    if (/single|in use|user data|locked|crashed|Target|closed/i.test(msg)) {
      throw new ApiError('PROFILE_LOCKED', `.linqx-profile 被其它浏览器进程占用(如 persistent.js 登录窗口未关闭),请关闭后重试: ${msg}`, { status: 409 });
    }
    throw new ApiError('BROWSER_ERROR', `cloakbrowser 启动失败: ${msg}`, { status: 502 });
  }
}

function onContextClosed() {
  if (closedByUs) return; // 主动关闭路径已清理
  logger.warn('[platform-orders] 浏览器意外断开,已清状态,下次请求自动重建');
  cleanupState();
}
...
async function closeInternal(reason) {
  ...
  logger.info({ reason }, '[platform-orders] 浏览器已关闭');
}
 ...
 * 任务超时(kind='timeout')或浏览器已断开时回收浏览器,其余错误(如登录态失效)
 * 保留浏览器复用
...
function status() {
  ...
  return {
    ...
    profileDir: config.platformProfileDir,
  };
}
...
/** 进程退出钩子(app.js shutdown 调用):关闭浏览器 + 清锁 */
