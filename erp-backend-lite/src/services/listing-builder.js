...
// ── 两阶段上架:图片处理阶段(2026-07 拆分) ───────────────
// prepareListing:执行 prepareBundleItems(含水印加工链),把转换后的 transformedItems 缓存
// 供 batch-image-poller 调用,不受 intervalSec 限速
// commitListing:从缓存的 transformedItems 直接调 opi.productImport,受 intervalSec 限速
// executeListing:保留原串行逻辑(供 Preview.vue 单条上架使用)
//
// prepareListing 返回 { localTaskId, transformedItems, error }
//   - error 非 null:prepareBundleItems 失败(已更新 follow_sell_tasks=FAILED)
//   - error 为 null:transformedItems 已就绪,已写入 follow_sell_task_payloads(transformed/opi_request)
export async function prepareListing(message, storeId, store) {
  ...
  // 执行 prepareBundleItems(含水印加工链:ai_rewrite → watermark → ai_poster → copy_ban)
  try {
    ...
    // 备份 transformed + opi_request payload
    try {
      db.prepare(
    ...
    return { localTaskId, transformedItems, error: null };
  } catch (e) {
    ...
    logger.warn({ localTaskId, err: errorMessage }, 'prepareListing failed');
    return { localTaskId, transformedItems: null, error: errorMessage };
  }
}
...
export async function commitListing(localTaskId, transformedItems, storeId, store) {
  ...
  try {
    ...
    // items 明细已在 prepareListing 写入,这里只更新状态为 failed
    try {
      const rows = db
        .prepare(`SELECT offer_id, name, price FROM follow_sell_task_items WHERE local_task_id=?`)
        .all(localTaskId);
      upsertTaskItems(
        localTaskId,
        rows.map((it) => ({
      ...
      logger.warn({ localTaskId, err: e2.message }, 'commitListing update items failed');
    }
    logger.warn({ localTaskId, err: errorMessage }, 'commitListing failed');
...
export async function executeListing(message, storeId, store) {
  // 串行执行两阶段(供 Preview.vue 单条上架使用,不走 batch poller)
  // 批量上架走 batch-image-poller(prepareListing) + batch-upload-poller(commitListing)
  const prepared = await prepareListing(message, storeId, store);
  if (prepared.error || !prepared.transformedItems) {
    return { localTaskId: prepared.localTaskId, ozonTaskId: null, error: prepared.error };
  }
  return commitListing(prepared.localTaskId, prepared.transformedItems, storeId, store);
}
