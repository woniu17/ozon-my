...
router.get('/admin/api/stock-refresh', (req, res, next) => {
  try {
    ...
    const rows = db
      .prepare(
        `SELECT * FROM stock_refresh_tasks ${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`
      )
      .all(...params, pageSize, offset);
    const total = db.prepare(`SELECT COUNT(*) as n FROM stock_refresh_tasks ${whereSql}`).get(...params).n;
    res.json(
      ok({
        items: rows.map((r) => ({
