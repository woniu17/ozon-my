<script setup>
// 图片更新任务列表页(2026-07)
// 展示所有 image_refresh_tasks,点击查看详情跳转到 ImageRefreshDetail.vue
import { reactive, onMounted } from 'vue';
import { useRouter } from 'vue-router';
import { getImageRefreshList } from '../api/imageRefresh.js';
...
async function loadList() {
  state.loading = true;
  try {
    const data = await getImageRefreshList({
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else-if="!state.items.length">
            <td colspan="9" class="empty">暂无图片更新任务</td>
          </tr>
