<script setup>
// 库存更新任务列表页(2026-07)
// 展示所有 stock_refresh_tasks,点击查看详情跳转到 StockRefreshDetail.vue
import { reactive, onMounted } from 'vue';
import { useRouter } from 'vue-router';
import { getStockRefreshList } from '../api/stockRefresh.js';
...
async function loadList() {
  state.loading = true;
  try {
    const data = await getStockRefreshList({
...
function openDetail(localTaskId) {
  router.push('/stock-refresh/' + encodeURIComponent(localTaskId));
}
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else-if="!state.items.length">
            <td colspan="10" class="empty">暂无库存更新任务</td>
          </tr>
          <tr v-for="t in state.items" :key="t.localTaskId">
            ...
            <td style="color:#1890ff;font-weight:600">{{ t.stockValue }}</td>
