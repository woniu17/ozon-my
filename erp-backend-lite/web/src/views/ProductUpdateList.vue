<script setup>
// 商品信息更新任务列表页(2026-07)
// 展示所有 product_update_tasks,点击查看详情跳转到 ProductUpdateDetail.vue
import { reactive, onMounted } from 'vue';
import { useRouter } from 'vue-router';
import { getProductUpdateList } from '../api/productUpdate.js';
...
async function loadList() {
  state.loading = true;
  try {
    const data = await getProductUpdateList({
...
function openDetail(localTaskId) {
  router.push('/product-update/' + encodeURIComponent(localTaskId));
}
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else-if="!state.items.length">
            <td colspan="10" class="empty">暂无商品信息更新任务</td>
          </tr>
