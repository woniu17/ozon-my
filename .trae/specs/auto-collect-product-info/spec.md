...
> V3 变更:在 V2 八类缓存基础上,(1)去掉搜索页采集,只保留店铺页和详情页;(2)新增中国大陆店铺检测层——进入店铺页/详情页时先提取卖家信息,通过预设规则或人工确认判定是否中国大陆店铺,只对中国大陆店铺的 SKU 触发自动采集。
...
需要一套**纯被动 + 可选自动翻页**的自动采集机制:用户浏览 Ozon 店铺页/详情页时,扩展先提取卖家信息并判定是否中国大陆店铺(规则自动 + 人工确认),只对中国大陆店铺的 SKU 自动采集全量跟卖信息(八类缓存:原 6 类 + 新增 marketStats/followSell)到 MongoDB,供后续 ERP 后台批量上架时直接命中缓存、零实时请求。同时提供 QX采集器浮动面板作为主控台,移植旧 MY采集器的触发方式、自动翻页、智能筛选、仅抓有销量功能。

**V3 新增动机**:用户只需采集中国大陆店铺的跟卖产品。搜索页无法可靠获取店铺信息,因此去掉搜索页采集,只保留店铺页和详情页——两者都能提取卖家信息(sellerSlug + sellerName + sellerId + companyInfo),据此判定是否中国大陆店铺。判定支持规则自动(已知列表/公司国家)与人工确认两种方式,结果存 MongoDB 跨设备共享。
...
- **新增 `autoCollect` action**:接收 `{sku, source, sellerSlug, depth, forceRefresh}`,编排 Gate 0 → Gate 0.5(中国大陆店铺) → 三层缓存检查(8 类)→ 买家页采集 → seller portal 采集 → 写 L1+L2+log
- **新增 Gate 0.5 中国大陆店铺检查**:在 Gate 0 之后、Step 1 之前,调 `checkStoreClassification(sellerSlug, ...)`,若 `onlyMainlandChinaStores` 开启且 `isMainlandChina !== true` 则跳过采集(reason=`non-mainland-china-store` 或 `unclassified-store`)
- **新增 `checkStoreClassification(slug, name, companyInfo)`**:三层查询 L1 chrome.storage → L2 MongoDB → 规则引擎,返回 `{isMainlandChina, classifiedBy}` 或 `null`(未分类)
- **新增 `classifyStoreByRules(slug, name, companyInfo, config)`**:规则引擎,5 条优先级规则(knownMainlandChinaSlugs → knownNonMainlandChinaSlugs → CJK-name → company-country=CN → company-country≠CN → null)
- **新增 `classifyStore` action**:人工确认,接收 `{slug, name, isMainlandChina}`,写 L1 + L2(`classifiedBy:'manual'`)
...
- **新增 chrome.storage 配置读写**:`jz-auto-collect-config` key,含 enabled/autoCollectRunning/depth/paused/pausedUntil/限速参数/stale 参数/每日计数/**onlyMainlandChinaStores/knownMainlandChinaSlugs/knownNonMainlandChinaSlugs**
...
- **新增内存计数器 + 环形缓冲**:今日 success/skipped/failed/antibot 计数 + 最近 50 条日志,byType 含 8 类,**bySource 只含 shop-page/pdp**(去掉 search-page),**byStoreClass 含 mainland-china/non-mainland-china/unclassified**,SW 启动时从 MongoDB 聚合初始化
...
- **store-detector.js**(新):店铺信息提取 UI + 中国大陆店铺检测区块渲染(待确认时显示 [标记中国大陆] [标记非中国大陆] 按钮,已分类时显示分类结果 + [重新分类]),挂 `window.QXStoreDetector`
- **panel.js + panel.css**:QX采集器面板,精简旧 panel.js(去掉本地桶/CSV/推送/关键词采集),保留主开关/自动翻页/仅抓有销量/智能筛选,新增**店铺检测区块**/**只采集中国大陆店铺开关**/今日统计/缓存命中(8 类,marketStats/followSell stale 标橙)/最近采集/熔断倒计时/强制刷新/查看ERP。样式前缀 `.qx-c-*`,类名 `QXCollectorPanel`,挂 `window.QXCollectorPanel`
...
- **新增 MongoDB collection `ozon_store_classification`**(新):中国大陆店铺分类结果(_id=sellerSlug,isMainlandChina:boolean|null,classifiedBy,companyInfo),跨设备共享
- **新增 6 个缓存端点**:GET/POST/DELETE `/ozon/cache/marketStats/:sku` + GET/POST/DELETE `/ozon/cache/followSell/:sku`;GET 返回 `stale` 字段
- **新增 4 个店铺分类端点**:GET/POST/DELETE `/ozon/store-classification/:slug` + GET `/ozon/store-classification`(列表,支持 isMainlandChina 筛选 + 分页)
...
- **Cache.vue 新增「店铺分类」tab**:分类列表(支持 isMainlandChina 筛选)+ 手动改分类
...
系统 SHALL 在用户浏览 Ozon 店铺页/详情页时,通过现有 IntersectionObserver / extractProductData 钩子,自动对每个遇到的 SKU(经「中国大陆店铺 + 仅抓有销量 + 智能筛选」过滤后)触发 `autoCollect` 消息,无需用户主动操作。**搜索页不触发采集**(V3 移除搜索页接入)。
...
### Requirement: 中国大陆店铺检测

系统 SHALL 在进入店铺页/详情页时先提取卖家信息(sellerSlug + sellerName + companyInfo),通过三层查询(L1 chrome.storage → L2 MongoDB → 规则引擎)判定是否中国大陆店铺,无法判定时返回 null 等待人工确认。分类结果存 MongoDB `ozon_store_classification` 跨设备共享。autoCollect action 入口新增 Gate 0.5,在 `onlyMainlandChinaStores` 开启时,非中国大陆店铺或未分类店铺的 SKU 跳过采集。
...
#### Scenario: 规则引擎 - 已知中国大陆店铺列表

- **WHEN** sellerSlug 在 `config.knownMainlandChinaSlugs` 列表中
- **THEN** 规则引擎返回 `{isMainlandChina:true, classifiedBy:'rule:known-list'}`,写 L1+L2,该店铺 SKU 可触发采集

#### Scenario: 规则引擎 - 已知非中国大陆店铺列表

- **WHEN** sellerSlug 在 `config.knownNonMainlandChinaSlugs` 列表中
- **THEN** 规则引擎返回 `{isMainlandChina:false, classifiedBy:'rule:known-list'}`,写 L1+L2,该店铺 SKU 跳过采集

#### Scenario: 规则引擎 - 公司国家为 CN

- **WHEN** companyInfo.country === 'CN'
- **THEN** 规则引擎返回 `{isMainlandChina:true, classifiedBy:'rule:company-country'}`,写 L1+L2

#### Scenario: 规则引擎 - 公司国家非 CN

- **WHEN** companyInfo.country 存在且 !== 'CN'
- **THEN** 规则引擎返回 `{isMainlandChina:false, classifiedBy:'rule:company-country'}`,写 L1+L2

#### Scenario: 规则引擎 - 无法判定

- **WHEN** 无规则匹配(不在已知列表、店名无 CJK、无公司国家信息)
- **THEN** 规则引擎返回 `{isMainlandChina:null, by:null}`,写 L2 记录(isMainlandChina=null),QX面板显示「待确认」+ [标记中国大陆] [标记非中国大陆] 按钮

#### Scenario: 三层查询缓存命中

- **WHEN** `checkStoreClassification(slug, ...)` 查 L1 chrome.storage.local 命中且 isMainlandChina !== null
- **THEN** 直接返回,不查 L2 不跑规则引擎

#### Scenario: 三层查询 L2 命中

- **WHEN** L1 未命中或 isMainlandChina === null,L2 MongoDB 命中且 isMainlandChina !== null
- **THEN** 写 L1 + 返回,不跑规则引擎

#### Scenario: 人工确认标记中国大陆

- **WHEN** QX面板显示「待确认」,用户点击 [✓ 标记中国大陆店铺]
- **THEN** content 调 `sendMessage('classifyStore', {slug, name, isMainlandChina:true})`,SW 写 L1 + L2(`classifiedBy:'manual'`),该页所有未采集 SKU 开始 autoCollect

#### Scenario: 人工确认标记非中国大陆

- **WHEN** 用户点击 [✗ 标记为非中国大陆店铺]
- **THEN** SW 写 L1 + L2(`classifiedBy:'manual', isMainlandChina:false`),该页不采集
...
#### Scenario: autoCollect Gate 0.5 - 中国大陆店铺通过

- **WHEN** autoCollect action 收到消息,Gate 0 通过,`checkStoreClassification` 返回 isMainlandChina === true
- **THEN** 继续执行 Step 1(三层缓存命中检查)

#### Scenario: autoCollect Gate 0.5 - 非中国大陆店铺跳过

- **WHEN** `config.onlyMainlandChinaStores === true` 且 `checkStoreClassification` 返回 isMainlandChina === false
- **THEN** SW 写 log `{status:'skipped', reason:'non-mainland-china-store', sellerSlug}`,返回 `{status:'skipped', reason:'non-mainland-china-store'}`,不真调

#### Scenario: autoCollect Gate 0.5 - 未分类店铺跳过

- **WHEN** `config.onlyMainlandChinaStores === true` 且 `checkStoreClassification` 返回 null(isMainlandChina === null)
- **THEN** SW 写 log `{status:'skipped', reason:'unclassified-store', sellerSlug}`,返回 `{status:'skipped', reason:'unclassified-store'}`,不真调,等待人工确认

#### Scenario: onlyMainlandChinaStores 关闭

- **WHEN** `config.onlyMainlandChinaStores === false`
- **THEN** Gate 0.5 不检查中国大陆店铺,所有 SKU 均可触发采集(仍受 Gate 0 + 三层缓存 + 销量/智能筛选过滤)
...
- **THEN** 写入日志 `{sku, source, sellerSlug, storeClassified:'mainland-china'|'non-mainland-china'|'unclassified', depth, status:'success'|'partial', results:[8 项], totalDuration, collectedAt}`

#### Scenario: 跳过采集 - 非中国大陆店铺

- **WHEN** 因 `onlyMainlandChinaStores` 开启且店铺判定为非中国而跳过
- **THEN** 写入日志 `{sku, source, sellerSlug, storeClassified:'non-mainland-china', status:'skipped', reason:'non-mainland-china-store'}`

#### Scenario: 跳过采集 - 未分类店铺

- **WHEN** 因 `onlyMainlandChinaStores` 开启且店铺未分类而跳过
...
- **WHEN** source 为 `pdp`
- **THEN** 直接调 `autoCollectOnSkuSeen`,不经过销量/智能筛选(用户主动访问 PDP,意图明确),但仍检查中国大陆店铺 Gate 0.5
...
系统 SHALL 提供 Ozon 页面上的浮动控制面板(QX采集器),作为自动采集的主控台,从旧 MY采集器面板移植触发方式、自动翻页、智能筛选、仅抓有销量功能,数据落地改为 MongoDB 8 类缓存。新增店铺检测区块与「只采集中国大陆店铺」开关。

#### Scenario: 面板展示与控制

- **WHEN** 用户在店铺页/详情页打开 QX采集器面板
- **THEN** 面板展示:主开关、店铺检测区块、今日统计(成功/跳过/失败/熔断)、缓存类目命中(8 类,marketStats/followSell stale 标橙色)、最近采集列表、自动翻页开关、仅抓有销量开关、**只采集中国大陆店铺开关**、智能筛选入口、强制刷新按钮、查看ERP按钮

#### Scenario: 店铺检测区块 - 已分类为中国

- **WHEN** 当前店铺 isMainlandChina === true
- **THEN** 区块显示「✓ 中国大陆店铺 (规则:cjk-name)」或「(人工确认)」,提供 [重新分类] 按钮

#### Scenario: 店铺检测区块 - 已分类为非中国

- **WHEN** 当前店铺 isMainlandChina === false
- **THEN** 区块显示「✗ 非中国大陆店铺 (规则:...)」或「(人工确认)」,提供 [重新分类] 按钮

#### Scenario: 店铺检测区块 - 待确认

- **WHEN** 当前店铺 isMainlandChina === null
- **THEN** 区块显示「⚠ 待确认」,提供 [✓ 标记中国大陆] [✗ 标记非中国大陆] 按钮;用户点击后调 `classifyStore` action
...
#### Scenario: 只采集中国大陆店铺开关

- **WHEN** 用户切换面板「只采集中国大陆店铺」开关
- **THEN** 写 `config.onlyMainlandChinaStores` 到 chrome.storage,SW 后续 Gate 0.5 据此决定是否检查中国大陆店铺;关闭时所有 SKU 均可触发采集
...
- **THEN** 调 `GET /ozon/store-classification?isMainlandChina=`,展示分类列表(sellerSlug/sellerName/isMainlandChina/classifiedBy/companyInfo/lastSeenAt),支持 isMainlandChina 筛选 + 手动改分类
