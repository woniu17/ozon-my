...
> V3 变更:在 V2 八类缓存基础上,(1)去掉搜索页接入任务;(2)新增中国店铺检测相关任务(extractSellerInfoFromShopPage、checkStoreClassification、classifyStoreByRules、classifyStore、store-detector.js、store_classification collection+端点);(3)autoCollect action 新增 Gate 0.5;(4)auto_collect_log 增加 sellerSlug/storeClassified 字段;(5)内存计数器新增 byStoreClass。
...
- [ ] Task 5: 新增店铺分类查询 + 规则引擎(NEW)
  - [ ] SubTask 5.1: 在 `service-worker.js` 新增 `classifyStoreByRules(slug, name, companyInfo, config)` 规则引擎函数,按优先级执行 4 条规则:
    - Rule 1: `config.knownChineseSlugs.includes(slug)` → `{isChinese:true, by:'rule:known-list'}`
    - Rule 2: `config.knownNonChineseSlugs.includes(slug)` → `{isChinese:false, by:'rule:known-list'}`
    - Rule 3: `companyInfo?.country === 'CN'` → `{isChinese:true, by:'rule:company-country'}`
    - Rule 4: `companyInfo?.country && companyInfo.country !== 'CN'` → `{isChinese:false, by:'rule:company-country'}`
    - 无匹配 → `{isChinese:null, by:null}`
  ...
  - [ ] SubTask 5.4: 新增 `checkStoreClassification(slug, name, companyInfo)` 三层查询函数:
    - L1: `chrome.storage.local.get('jz-store-class-${slug}')`,命中且 isChinese !== null → 返回
    - L2: `_erpStoreClassGet(slug)`,命中且 isChinese !== null → 写 L1 + 返回
    - 规则引擎: `classifyStoreByRules(...)`,isChinese !== null → 写 L1 + L2 + 返回
    - 未分类: 写 L2 记录(isChinese=null,等待人工确认)→ 返回 null
  - [ ] SubTask 5.5: 新增 `case 'checkStoreClassification'` 消息路由,接收 `{slug, name, companyInfo?}`,返回 `{isChinese, classifiedBy} | null`
  - [ ] SubTask 5.6: 新增 `case 'classifyStore'` 消息路由(人工确认),接收 `{slug, name, isChinese}`,构造 record(`classifiedBy:'manual'`, `classifiedAt:new Date()`),写 L1 chrome.storage + L2 MongoDB,返回 `{ok:true}`

- [ ] Task 6: 新增 `autoCollect` action(8 类编排 + Gate 0.5 中国店铺检查)
  ...
  - [ ] SubTask 6.3: **Gate 0.5 中国店铺检查**(NEW):调 `checkStoreClassification(sellerSlug, ...)`,若 `config.onlyChineseStores === true` 且 `cls?.isChinese !== true`,写 log + 返回 `{status:'skipped', reason: cls?.isChinese === false ? 'non-chinese-store' : 'unclassified-store'}`
...
- [ ] Task 7: 新增 `ozon_auto_collect_log` collection 接口
  - [ ] SubTask 7.1: 在 `erp-backend-lite/src/modules/cache.js` 新增 `GET /admin/api/auto-collect/stats`:聚合 `ozon_auto_collect_log`,返回今日/本周的 `{total, success, partial, failed, skipped, antibot, byType:{card,detail,composer,entrypoint,search,bundle,marketStats,followSell}, bySource:{shop-page,pdp}, byStoreClass:{chinese,non-chinese,unclassified}}`
...
- [ ] Task 9: 新增 `ozon_store_classification` collection + 4 个端点(NEW)
  ...
  - [ ] SubTask 9.2: 在 `erp-backend-lite/src/modules/cache.js` 新增 `GET /ozon/store-classification/:slug`:按 slug 查询单条分类记录,返回 `{sellerSlug, sellerName, isChinese, classifiedBy, companyInfo, lastSeenAt, lastSeenUrl}` 或 404
  - [ ] SubTask 9.3: 新增 `POST /ozon/store-classification/:slug`:upsert 分类记录,字段含 `sellerSlug/sellerName/isChinese/classifiedBy/classifiedAt/companyInfo/lastSeenAt/lastSeenUrl`;已存在时更新 isChinese/classifiedBy/classifiedAt 并刷新 lastSeenAt
  - [ ] SubTask 9.4: 新增 `GET /ozon/store-classification`:列表查询,支持 query `isChinese=true/false/null` 筛选 + `keyword`(匹配 sellerName/sellerSlug)+ `currentPage/pageSize` 分页,按 lastSeenAt DESC
  - [ ] SubTask 9.5: 新增 `DELETE /ozon/store-classification/:slug`:删除单条分类记录
  - [ ] SubTask 9.6: 为 `ozon_store_classification` 创建索引:`{isChinese:1}`、`{sellerName:1}`、`{lastSeenAt:-1}`
...
- [ ] Task 16: ozon-data-panel.js 店铺页接入(含卖家信息提取 + 中国店铺检测)
  ...
  - [ ] SubTask 16.2: 调 `sendMessage('checkStoreClassification', {slug, name, companyInfo})`,获取 `{isChinese, classifiedBy} | null`,更新 QX面板店铺检测区块状态
...
- [ ] Task 17: ozon-product.js 详情页接入(含 sellerSlug 提取 + 中国店铺检测)
  ...
  - [ ] SubTask 17.3: 调用 `autoCollectOnSkuSeen(String(_product.sku), 'pdp', sellerSlug)`(详情页无 panel 数据,不经过销量/智能筛选,但仍检查中国店铺 Gate 0.5)
...
- [ ] Task 20: 创建 qx-collector/store-detector.js(NEW)
  ...
  - [ ] SubTask 20.2: 实现 `renderStoreDetectionBlock(container, {slug, name, isChinese, classifiedBy})` 函数,根据 isChinese 渲染不同状态:
    - `true`:显示「✓ 中国店铺 (规则:xxx)」或「(人工确认)」+ [重新分类] 按钮
    - `false`:显示「✗ 非中国店铺 (规则:xxx)」或「(人工确认)」+ [重新分类] 按钮
    - `null`:显示「⚠ 待确认」+ [✓ 标记中国] [✗ 标记非中国] 按钮
    - 未检测:显示「— 等待页面加载 —」
  - [ ] SubTask 20.3: [标记中国] 按钮点击 → `sendMessage('classifyStore', {slug, name, isChinese:true})` → 刷新区块状态 → 通知该页所有未采集 SKU 开始 autoCollect
  - [ ] SubTask 20.4: [标记非中国] 按钮点击 → `sendMessage('classifyStore', {slug, name, isChinese:false})` → 刷新区块状态
  - [ ] SubTask 20.5: [重新分类] 按钮点击 → 区块回到「待确认」状态(不清除 L1/L2 记录,仅 UI 状态变化,用户重新标记后覆盖)

- [ ] Task 21: 创建 qx-collector/panel.js + panel.css(含店铺检测区块 + 只采集中国店铺开关)
  ...
  - [ ] SubTask 21.3: 新增**「只采集中国店铺」开关**(checkbox),读写 `config.onlyChineseStores`
  ...
  - [ ] SubTask 21.6: 回调:`onToggleRunning`(切换 autoCollectRunning + 联动 AutoScroller)、`onAutoScrollToggle`、`onSalesFilterChange`、`onOnlyChineseStoresChange`、`onFilterOpen/Save/Reset`、`onForceRefresh`、`onViewErp`
...
- [ ] Task 22: SW 新增 autoCollect* + checkStoreClassification/classifyStore action(供面板调用)
  - [ ] SubTask 22.1: `case 'autoCollectGetConfig'` 返回 `{enabled, autoCollectRunning, depth, paused, pausedUntil, todayCount, perDayLimit, todayDate, marketStatsStaleMs, followSellStaleMs, onlyChineseStores, knownChineseSlugs, knownNonChineseSlugs}`
  - [ ] SubTask 22.2: `case 'autoCollectSetConfig'` 接收 `{enabled?, autoCollectRunning?, paused?, depth?, marketStatsStaleMs?, followSellStaleMs?, onlyChineseStores?, knownChineseSlugs?, knownNonChineseSlugs?}`,写 chrome.storage + 推送 `configChanged` 消息
  - [ ] SubTask 22.3: `case 'autoCollectGetStats'` 返回内存计数器 `{today:{success,skipped,failed,antibot,total}, byType(8类), bySource:{shop-page,pdp}, byStoreClass:{chinese,non-chinese,unclassified}, successRate}`
  ...
  - [ ] SubTask 22.7: `case 'classifyStore'`(Task 5.6 已实现,此处确认面板可调用);人工标记为中国店铺后,向当前 tab 发消息触发该页未采集 SKU 开始 autoCollect
  - [ ] SubTask 22.8: SW 内存计数器 + 环形缓冲:今日 success/skipped/failed/antibot 计数 + 最近 50 条日志,byType 含 8 类,bySource 含 shop-page/pdp,byStoreClass 含 chinese/non-chinese/unclassified,SW 启动时从 MongoDB auto_collect_log 聚合初始化

- [ ] Task 23: ozon-data-panel.js / ozon-product.js 接入 QX面板
  - [ ] SubTask 23.1: ozon-data-panel.js 替换 `JZCollectorPanel.create` 为 `QXCollectorPanel.create`,回调对接 autoCollectRunning/AutoScroller/智能筛选/仅抓有销量/**只采集中国店铺**
...
- [ ] Task 25: popup.html 新增「自动采集」简化区块
  ...
  - [ ] SubTask 25.3: 复用现有 popup.css 风格,不引入新样式文件;不暴露深度/限速/自动翻页/筛选/只采集中国店铺(这些在 QX面板)
...
- [ ] Task 28: Cache.vue 新增「自动采集」tab + 「店铺分类」tab + overview 适配 8 类
  ...
  - [ ] SubTask 28.4: 店铺分类 tab - 新增分类列表:支持 isChinese 筛选 + keyword 搜索 + 分页(调 `getStoreClassificationList`),表格列:sellerSlug/sellerName/isChinese/classifiedBy/companyInfo/lastSeenAt;支持手动改分类(调 `updateStoreClassification`)
...
- [ ] Task 30: 端到端验证
  - [ ] SubTask 30.1: 店铺页浏览中国店铺 10 个 SKU → 查 MongoDB 八类缓存各有 10 条(detail 为空,因详情页未访问),auto_collect_log 有 10 条(storeClassified=chinese)
  - [ ] SubTask 30.2: 详情页浏览中国店铺 10 个 SKU → 八类缓存各有 10 条(含 detail)
  - [ ] SubTask 30.3: 店铺页浏览非中国店铺 → 不触发 autoCollect,log 记 `non-chinese-store`(storeClassified=non-chinese)
  - [ ] SubTask 30.4: 店铺页浏览未分类店铺 → QX面板显示「待确认」,不触发 autoCollect,log 记 `unclassified-store`(storeClassified=unclassified)
  - [ ] SubTask 30.5: 人工标记未分类店铺为中国店铺 → 该页 SKU 开始采集
  - [ ] SubTask 30.6: 人工标记中国店铺为非中国店铺 → 该页后续 SKU 不采集
  - [ ] SubTask 30.7: 规则自动判定:CJK 店名(如「三发力耶」) → 自动标记为中国店铺(classifiedBy=rule:cjk-name)
  - [ ] SubTask 30.8: 规则自动判定:knownNonChineseSlugs 列表中的 slug → 自动标记为非中国店铺
  ...
  - [ ] SubTask 30.15: QX面板「只采集中国店铺」开关关闭 → 非中国店铺也采集
  ...
  - [ ] SubTask 30.22: ERP overview 页「店铺分类」tab 可见分类列表(支持 isChinese 筛选),可手动改分类
