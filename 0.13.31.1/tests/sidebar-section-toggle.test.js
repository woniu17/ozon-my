...
const compactCollapsedBody = searchCss.slice(compactCollapsedIndex, searchCss.indexOf('}', compactCollapsedIndex));
...
for (const group of [searchGroup, dataPanelGroup]) {
  ...
  const callerIndex = Math.max(group.indexOf('content/ozon-search.js'), group.indexOf('content/ozon-data-panel.js'));
