...
assert.deepStrictEqual(extractOzonVideoFromSources([galleryWithNewVideoShape]), {
...
assert.deepStrictEqual(extractOzonVideoFromSources([galleryWithStreamOnlyVideo]), {
  mp4: null,
  cover: null,
});
...
assert.strictEqual(extractOzonMp4FromText(escapedState), 'https://cdn3.ozone.ru/s3/video-3/state-video.mp4?x=1');

assert.strictEqual(extractOzonMp4FromSources([{ videos: [{ url: 'https://cdn.ozone.ru/video.m3u8' }] }]), null);
