...
(() => {
  ...
  if (typeof globalThis.JzBrowserAgentActions.registerDynamicCapability === 'function') {
    globalThis.JzBrowserAgentActions.registerDynamicCapability('ozon.seller_collect', async () => {
      try {
  ...
  function buildHotProductsUrl(params) {
    ...
    const query = category || (source === 'china_pavilion' ? 'товары из китая' : 'популярные товары');
  ...
  async function enrichVariant(raw) {
    ...
    try {
      const response = await new Promise((resolve) => {
        chrome.runtime.sendMessage({ action: 'searchVariants', sku: String(raw.sku) }, (result) => resolve(result));
      });
  ...
  globalThis.JzBrowserAgentActions.register('collect.hot_products', async (job, context) => {
  ...
  globalThis.JzBrowserAgentActions.register('ozon.collect_variant', async (job, context) => {
    ...
    const response = await new Promise((resolve) => {
      chrome.runtime.sendMessage({ action: 'searchVariants', sku, noProxy: true }, (result) => {
  ...
  globalThis.JzBrowserAgentActions.register('ozon.market_data', async (job, context) => {
    const sku = String(job?.params?.sku || '').trim();
    ...
    const response = await new Promise((resolve) => {
      chrome.runtime.sendMessage({ action: 'getMarketStats', sku, period, noProxy: true }, (result) => {
        if (chrome.runtime.lastError) {
          resolve({ ok: false, error: chrome.runtime.lastError.message });
        } else {
          resolve(result);
  ...
  globalThis.JzBrowserAgentActions.register('collect.product_detail', async (job, context) => {
    const params = job?.params || {};
