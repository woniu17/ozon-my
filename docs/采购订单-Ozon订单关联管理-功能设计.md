...
2. 多张 Ozon 订单可复用同一张 1688 采购单（一单多关联，实测见§7.1），金额需按关联行分摊——需要中间层拆账
...
CREATE TABLE IF NOT EXISTS purchase_order_link (
  ...
  -- 成本分摊结果（一单多关联拆账落点）
  allocated_amount    REAL DEFAULT 0,           -- 分摊采购金额(含运费)
...
CREATE TABLE IF NOT EXISTS supplier_product (
  ...
  source_url          TEXT,                     -- https://detail.1688.com/offer/997687875219.html
  source_item_id      TEXT,                     -- 1688 offerId(采购补全自动写入)
  title               TEXT,
  ...
  gmt_create          TEXT,
  gmt_modified        TEXT,
  UNIQUE(store_id, COALESCE(offer_id,''), source_item_id)
);
```
...
Ozon同步 → [wait_process 待处理·未采购]                    │
   │ 提交采购信息(单号补全/手填) → 直接进入待打单发货          │
   ▼                    (采购到货与否由轨迹在列表中展示,      │
[wait_ship 待打单发货]     不阻塞流转)                      │
   │  ↑ 国内轨迹签收=到货(package.arrived_at) → 提示可打包    │
   │                                                        │
   │ ── 打印Ozon面单 ──> [ship_success 交运]                  │
   │                       waybill_printed, 我送到Ozon网点    │
   ▼                            ▼                           │
...
**提交采购信息即流转**：填写采购信息（无论是否已有快递单号）后包裹从待处理直接进入「待打单发货」。采购的到货进度不阻塞状态流转，在列表中以轨迹/到货标记展示（等收货→已到货→可打包）。

与妙手状态机的差异：妙手把"采购+出货登记+推送货代"合并为一次「提交代打包」且以出货信息齐备为流转条件；本设计以**采购录入完成**为流转条件，更贴合"先登记采购、到货后自行打单"的个人节奏。Ozon侧状态联动：
...
* `package.arrived_at`：关联采购单全部 `signed`（或国内轨迹显示签收）时回填——列表展示"已到货·可打包"标记（仅提示，不阻塞打单操作，由人自行判断）
...
**平台内部标识**：`1688` / `yangkeduo`（拼多多）/ `taobao` / `tmall` / `jingdong`…（采购记录页筛选口径与之一致）。

##### 1688 补全链路（实测：单次保存即完成）

填入 `5127660720062029909` 后点保存 → 提交 `manualRelateOrderPackagePurchaseOrders`（body 仅 `purchasePlatform:"1688"` + 单号 + `isAutoRsyncPurchasePayment=1`）→ **后端拿单号实时调1688开放接口**拉取订单详情并落库，自动补全字段：
...
**与1688的关键差异**：PDD**一次保存不落库**，而是把拉到的订单详情回填到表单，弹窗保留等用户确认，第二次保存才成功。

```
填入 260829-516906094883751 → 第一次点保存
→ 请求200但弹窗不关，表单被自动回填：
   产品实付 0.00→9.49 | 采购账号 PCC01 | 卖家账号 钢拓高端五金工具
→ 第二次点保存 → {"result":"success","successCount":1,"failCount":0}
```

**第二次提交的请求体携带完整前置字段**（区别于1688只传单号）：

| 请求字段                                              | 实测值                               | 说明                            |
| ------------------------------------------------- | --------------------------------- | ----------------------------- |
| `purchasePlatform`                                | `yangkeduo`                       | 拼多多内部标识                       |
| `purchaseOrderSn`                                 | 260829-516906094883751            | PDD单号                         |
| `purchaseOrderPayment`                            | 9.49                              | PDD实付                         |
...
| `purchaseOrderWaybillCode`                        | JT5520524391343                   | 快递单号                          |
...
**提交后流转**：`checkPackagesIsAutoMoveWaitShip` 判定（采购信息齐备+已有国内快递单号）后包裹自动流转。**复刻版语义：提交采购信息（无论何种模式、是否有快递单号）即从「待处理」直接流转到「待打单发货」**；国内快递单号由物流同步后续自动补齐，到货进度在列表中以轨迹/标记展示（见§5.1、§7.3）。
...
```
待处理(未采购) ──提交采购信息──> 待打单发货(直接流转)
                                  │ 采购物流同步: 上家发货→国内轨迹(揽收/中转/派送)
                                  │ → 轨迹"已签收"=到货(arrived_at) 列表标记"已到货·可打包"
                                  ▼
                       (到货后)自行打包 [打印Ozon面单] /v2/posting/fbs/package-label (PDF, 20单/批)
                                  │ Ozon跟踪号即posting_number(跨境线上物流,无需申请)
                                  ▼
                       我送到Ozon网点/揽收点交运 → Ozon status=delivering
                                  ▼
                       妥投 delivered → 已发货完结 → 回款结算(报表核对)
```

要点：

* **提交采购信息即流转**到待打单发货；到货进度不阻塞状态，以列表标记（等收货/已到货·可打包）提示何时该打单
...
复用 `ozon-webhook` 的 `feishu-notify.js` 模式：异常聚合日报 + 高优（延迟发货）即时通知 + **到货提醒**（采购签收→待打单列表标记"可打包"，可选）。
...
| `qxqx/.env` 采集脚本模式              | 1688订单详情拉取/物流同步脚本同款环境（ERP\_BASE\_URL/ERP\_API\_KEY）                                                           |
...
**验收**：订单自动同步；模式B录入后列表正确展示采购金额与预估利润，包裹直接流转待打单发货。

### Phase 2 — 采购补全与记录页

1. 提交采购信息**模式A**：1688/PDD采购单号自动补全（拉取订单详情落库+金额自动同步）
2. 采购记录页完整版（Tab/搜索/关联状态/取消关联/1688货源信息展示）
3. 国内物流同步（1688优先）+ 三类异常监控 + 飞书通知 + 到货标记回填

**验收**：填1688单号保存后实付/上家/采购账号/货源图自动补全；轨迹停滞24h触发通知；签收后待打单发货列表显示"已到货·可打包"标记。

### Phase 3 — 发货执行与体验完善

1. 打印Ozon面单（`/v2/posting/fbs/package-label`）+ "已到货"筛选驱动打单队列
...
* 多对一样本：1688单 `5127395654065010728`（锯片×3）同时关联包裹 `MS20260829021743112`（×1）与 `MS20260829021743131`（×2）→ 验证一单多关联分摊
