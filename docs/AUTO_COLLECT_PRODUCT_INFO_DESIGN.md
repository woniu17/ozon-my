...
> V3 变更:在 V2 八类缓存基础上,(1)去掉搜索页采集,只保留店铺页和详情页;(2)新增中国店铺检测层——进入店铺页/详情页时先提取卖家信息,通过预设规则或人工确认判定是否中国店铺,只对中国店铺的产品触发自动采集。

## 1. 总览

### 1.1 目标

用户浏览 Ozon 店铺页或详情页时,扩展先提取卖家信息并判定是否中国店铺(规则自动 + 人工确认),只对中国店铺的 SKU 自动采集全量跟卖信息(八类缓存:card / detail / composer / entrypoint / search / bundle / marketStats / followSell)到 MongoDB,供后续 ERP 后台对「数据就绪」的 SKU 发起批量上架。
...
| 中国店铺过滤 | 规则自动 + 人工确认 | 先规则判定,无法判定时人工确认;分类结果存 MongoDB 跨设备共享 |
...
| 过滤策略 | 中国店铺 + 仅抓有销量 + 智能筛选(18 字段) | 三层过滤,层层缩减 |
...
│  │  → 返回 {isChinese: true/false/null, classifiedBy}   │        │
│  └─────────────────────────────────────────────────────┘        │
│         │ (isChinese === true 时继续)                           │
│         ▼                                                       │
│  ┌─────────────────────────────────────────────────────┐        │
...
│  ozon_store_classification(新) ← 中国店铺分类结果               │
...
### 3.4 新增 ozon_store_classification(中国店铺分类)
...
```javascript
{
  ...
  isChinese: boolean | null,  // true=中国,false=非中国,null=未分类(等待人工确认)
...
```javascript
{
  ...
  sellerSlug: "string",
  storeClassified: "chinese" | "non-chinese" | "unclassified",
  depth: "Full",
  status: "success" | "partial" | "failed" | "skipped" | "antibot" | "non-chinese-store",
  results: [
...
// key: jz-auto-collect-config
{
  ...
  // 中国店铺检测(新增)
  onlyChineseStores: true,       // 只采集中国店铺(主开关)
  knownChineseSlugs: [],         // 已知中国店铺 slug 列表(用户手动添加/规则学习)
  knownNonChineseSlugs: [],      // 已知非中国店铺 slug 列表
}
```
...
| 优先级 | 规则名 | 判定逻辑 | 结果 |
|---|---|---|---|
| 1 | `known-list` | sellerSlug 在 `knownChineseSlugs` 列表中 | isChinese=true |
| 2 | `known-list` | sellerSlug 在 `knownNonChineseSlugs` 列表中 | isChinese=false |
| 3 | `cjk-name` | sellerName 含 CJK 汉字(`[\u4e00-\u9fff]`) | isChinese=true |
| 4 | `company-country` | companyInfo.country === 'CN' | isChinese=true |
| 5 | `company-country` | companyInfo.country 存在且 !== 'CN' | isChinese=false |
| 6 | — | 无规则匹配 | isChinese=null(等待人工确认) |

**规则代码**:
```javascript
function classifyStoreByRules(slug, name, companyInfo, config) {
  // Rule 1-2: known list
  if (config.knownChineseSlugs?.includes(slug)) return { isChinese: true, by: 'rule:known-list' };
  if (config.knownNonChineseSlugs?.includes(slug)) return { isChinese: false, by: 'rule:known-list' };
  // Rule 3: CJK characters in name
  if (name && /[\u4e00-\u9fff]/.test(name)) return { isChinese: true, by: 'rule:cjk-name' };
  // Rule 4-5: company country
  if (companyInfo?.country) {
    if (companyInfo.country === 'CN') return { isChinese: true, by: 'rule:company-country' };
...
checkStoreClassification(slug, name, companyInfo?):
  1. 查 L1 chrome.storage.local(key: jz-store-class-{slug})
     → 命中且 isChinese !== null → 返回
  2. 查 L2 MongoDB ozon_store_classification
     → 命中且 isChinese !== null → 写 L1 + 返回
  3. 跑规则引擎 classifyStoreByRules(slug, name, companyInfo, config)
     → isChinese !== null → 写 L1 + L2 + 返回
  4. isChinese === null → 写 L2 记录(isChinese=null,等待人工确认)→ 返回 null
```

### 4.5 人工确认

当 `checkStoreClassification` 返回 `null`(isChinese=null)时:
...
2. 用户点击按钮 → `sendMessage('classifyStore', {slug, name, isChinese: true/false})`

3. SW 写 L1 chrome.storage + L2 MongoDB(`classifiedBy: 'manual'`)

4. 如果标记为中国店铺 → 该页所有未采集 SKU 开始 autoCollect

5. 如果标记为非中国店铺 → 该页不采集

### 4.6 与 autoCollect 的衔接

autoCollect action 入口新增 **Gate 0.5: 中国店铺检查**:
...
autoCollect(sku, source, sellerSlug, ...):
  ...
  Gate 0.5: 中国店铺检查
    ...
    if (config.onlyChineseStores && cls.isChinese !== true) {
      writeLog({status:'skipped', reason: cls.isChinese === false ? 'non-chinese-store' : 'unclassified-store'})
      return {status:'skipped', reason: cls.isChinese === false ? 'non-chinese-store' : 'unclassified-store'}
    }
  Step 1-7: ...(同 V2)
```
...
| detail | **不采** | 店铺页无 PDP |
| composer | 采(中国店铺 + 筛选通过时) | buyerPage 真调 |
...
// SW → content (response)
{
  ...
  reason: "cooldown" | "all-cached" | "daily-limit" | "disabled"
          | "non-chinese-store" | "unclassified-store",
  results: [
    { type: "card",        hit: true,  source: "content" },
    { type: "detail",      hit: false, source: "n/a" },
...
async function checkStoreClassification(slug, name, companyInfo) {
  ...
  if (l1[l1Key]?.isChinese !== undefined && l1[l1Key]?.isChinese !== null) {
    return l1[l1Key];
  }
  ...
  if (rule.isChinese !== null) {
    const record = {
      sellerSlug: slug, sellerName: name, isChinese: rule.isChinese,
  ...
  if (!l2) {
    const record = {
      sellerSlug: slug, sellerName: name, isChinese: null,
...
| isChinese | 显示 | 可操作 |
|---|---|---|
| `true` | ✓ 中国店铺 (规则:xxx) 或 (人工确认) | [重新分类] |
| `false` | ✗ 非中国店铺 (规则:xxx) 或 (人工确认) | [重新分类] |
| `null` | ⚠ 待确认 | [✓ 标记中国] [✗ 标记非中国] |
| 未检测 | — 等待页面加载 — | 无 |
...
| action | 入参 | 返回 | 说明 |
|---|---|---|---|
| `checkStoreClassification` | `{slug, name, companyInfo?}` | `{isChinese, classifiedBy} \| null` | 查询店铺分类 |
| `classifyStore` | `{slug, name, isChinese}` | `{ok}` | 人工确认分类 |
| `autoCollectGetConfig` | - | `{enabled, autoCollectRunning, depth, paused, pausedUntil, todayCount, perDayLimit, todayDate, onlyChineseStores, ...}` | 当前配置 |
| `autoCollectSetConfig` | `{enabled?, autoCollectRunning?, paused?, depth?, onlyChineseStores?, ...}` | `{ok}` | 修改配置 |
...
collectAutoIfMatched(productId, card, info, data, panel, source, sellerSlug):
  ...
  // Layer 1: 中国店铺(已在 autoCollect Gate 0.5 检查,此处不重复)
  // Layer 2: 仅抓有销量
...
**注**:详情页跳过 Layer 2/3,但保留 Layer 1(中国店铺检查在 SW Gate 0.5)。
...
qx-ozon/content/qx-collector/
  ...
  store-detector.js  ← 店铺信息提取 + 中国店铺检测 UI(新)
```
...
| GET | `/ozon/store-classification` | 列表(支持 isChinese 筛选 + 分页) |
...
| P0 | SW 新增 autoCollect action(含 Gate 0.5 中国店铺检查) | service-worker.js | P0 闸门 + 缓存 + 分类 |
...
| 中国店铺规则误判 | 规则只做正向判定(CJK 字符/已知列表),无把握的走人工确认;用户可随时重新分类 |
...
- [ ] 店铺页浏览中国店铺 10 SKU → 八类缓存各 10 条(detail 空),log 10 条
- [ ] 详情页浏览中国店铺 10 SKU → 八类缓存各 10 条(含 detail)
- [ ] 店铺页浏览非中国店铺 → 不触发 autoCollect,log 记 `non-chinese-store`
- [ ] 店铺页浏览未分类店铺 → QX面板显示「待确认」,不触发 autoCollect
- [ ] 人工标记未分类店铺为中国店铺 → 该页 SKU 开始采集
- [ ] 人工标记中国店铺为非中国店铺 → 该页后续 SKU 不采集
- [ ] 规则自动判定:CJK 店名 → 自动标记为中国店铺
- [ ] 规则自动判定:knownNonChineseSlugs → 自动标记为非中国店铺
...
- [ ] ERP overview 页「店铺分类」tab 可见分类列表(支持 isChinese 筛选)
